...
def pop():
    popup=SPLASH('amobox.xml',ADDON.getAddonInfo('path'),'DefaultSkin',close_time=34)
    popup.doModal()
    del popup
#-----------------------------------------------------------------------------------------------------------------    
...
def ADDITIONAL_TOOLS():
    addDir('Delete Builds From Device','url',81,'','','Delete Build')
    addDir('Wipe My Setup (Fresh Start)','url',82,'','','Wipe your special XBMC/Kodi directory which will revert back to a vanillla build.')
    addDir('Convert Physical Paths To Special',HOME,83,'','','Convert Physical Paths To Special')
    addDir('Check my download location','url',84,'','','Force close kodi, to be used as last resort')
    addDir('Force Close Kodi','url',85,'','','Force close kodi, to be used as last resort')
#---------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------
#Get keyboard
...
#    dialog.ok("[COLOR=red][B]VERY IMPORTANT![/COLOR][/B]", 'If you plan on creating a backup to share [COLOR=lime]ALWAYS[/COLOR] make', 'sure you\'ve deleted your addon_data folder as uninstalling', 'an addon does not remove personal data such as passwords.')             
    addDir('[COLOR=lime]Full Backup[/COLOR]','url',61,'Backup.png','','Back Up Your Full System')
    addDir('Backup Just Your Addons','addons',62,'Backup.png','','Back Up Your Addons')
    addDir('Backup Just Your Addon UserData','addon_data',62,'Backup.png','','Back Up Your Addon Userdata')
    addDir('Backup Guisettings.xml',GUI,64,'Backup.png','','Back Up Your guisettings.xml')
    if os.path.exists(FAVS):
        addDir('Backup Favourites.xml',FAVS,64,'Backup.png','','Back Up Your favourites.xml')
    if os.path.exists(SOURCE):
        addDir('Backup Source.xml',SOURCE,64,'Backup.png','','Back Up Your sources.xml')
    if os.path.exists(ADVANCED):
        addDir('Backup Advancedsettings.xml',ADVANCED,64,'Backup.png','','Back Up Your advancedsettings.xml')
    if os.path.exists(KEYMAPS):
        addDir('Backup Advancedsettings.xml',KEYMAPS,64,'Backup.png','','Back Up Your keyboard.xml')
    if os.path.exists(RSS):
        addDir('Backup RssFeeds.xml',RSS,64,'Backup.png','','Back Up Your RssFeeds.xml')
#---------------------------------------------------------------------------------------------------
#Create restore menu
