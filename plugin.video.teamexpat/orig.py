...
AddonID = 'plugin.video.teamexpat'
...
ServerURL = "http://go.teamexpat.ninja/get.php?username=%s&password=%s&type=m3u&output=hls"%(Username,Password,)
...
def AddDir(name, url, mode, iconimage, description="", isFolder=True, background=None):
    ...
    print name.replace('-[US]','').replace('-[EU]','').replace('[COLOR limegreen]','').replace('[/COLOR]','').replace(' (G)','')+'='+a
