...
ADDON=xbmcaddon.Addon(id='plugin.video.teamexpat')
...
def Fix_Special(url):
    dialogprocess.create("STV","Renaming paths...",'', 'Please Wait')
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dialogprocess.create("STV","Archiving...",'', 'Please Wait')
...
def Destroy_Path(path):
    dialogprocess.create("STV ","Cleaning...",'', 'Please Wait')
...
def Check_Path():
    if zip=='':
     if dialog.yesno("STV Wizard", "You Have Not Set Your Storage Path", 'Set The Storage Path Now ?',''):
        ADDON.openSettings()
        print '######### ZIP DIRECTORY #########'
...
def RestoreIt():
    ...
    dialog = xbmcgui.Dialog()
    if zip == '':
        dialog.ok('STV Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    dialogprocess.create("STV Wizard","Restoring",'', 'Please Wait')   
    lib=xbmc.translatePath(os.path.join(zip,'backup.zip'))
    Read_Zip(lib)
    dialogprocess.create("STV Wizard","Checking ",'', 'Please Wait')
    ...
    dialogprocess.close()
    dialog.ok("STV Wizard", "All Done, DONT PRESS OK", "Wait a 5 minutes and pull the Power","")	
def Backupzip():  
    if zip == '':
        dialog.ok('STV Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    ...
    dialogprocess.create("STV Wizard","Backing Up",'', 'Please Wait')
    ...
    for base, dirs, files in os.walk(to_backup):
        for file in files:
            for_progress.append(file) 
            progress = len(for_progress) / float(N_ITEM) * 100  
            dialogprocess.update(int(progress),"Backing Up",'[COLOR limegreen]%s[/COLOR]'%file, 'Please Wait')
            fn = os.path.join(base, file)
            if not 'temp' in dirs:
                if not 'plugin.program.xyz' in dirs:
                   import time
    ...
    zipobj.close()
    dialogprocess.close()
    dialog.ok("STV Wizard", "You Are Now Backed Up", '','')
